...
<script type="text/javascript">
  ...
  var uploadPosterUrl = "<?php echo U('Common/uploadVideoPoster');?>";//后台PHP处理地址
    ...
    <div class="col-md-12" style="padding: 10px 40% 10px 40%;">
        <a href="<?php echo U('addClassify',array('id'=>'根'));?>" class="button button-3d button-primary button-rounded center-bolck">添加根栏目</a>
    </div>
    <table class="table table-hover table-condensed col-md-12">
        <thead>
            <tr>
                <th style="width: 5%">编号</th>
                <th style="width: 85%">栏目名</th>
        ...
        <tbody>
            <?php if($classifyLists): if(is_array($classifyLists)): foreach($classifyLists as $key=>$v): ?><tr>
                        ...
                        <td>
                            <?php if($v["html"]): ?><a href="#" onclick="javascript:return false;" class="not">　　　</a> 
                            <?php else: ?>
                                <a href="<?php echo U('addClassify',array('id'=>$v['id']));?>">[添加]</a> |<?php endif; ?>
...
<script type="text/javascript" src="/Public/Uploadify/jquery.uploadify.min.js"></script>
    ...
    $('a.delete').on('click', function() {
        if (confirm('确认删除么')) {
            ...
            //后台ajax删除 
            $.post("<?php echo U('deleteClassify');?>", {
                'id': id
            }, function(data) {
                if (data.status) {
                    tishi(data.msg);
