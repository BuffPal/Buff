...
<nav class="navbar navbar-default navbar-fixed-top">
    ...
    <!-- 音乐详细页样式 -->
    <link rel="stylesheet" type="text/css" href="/Public/css/Index/M_lists.css">
    <div class="container p0" style="margin-top: 80px;">
        <div class="row">
            <!-- 手风琴分类 -->
            <div class="col-md-2 p0">
                <div class="panel-group" id="accordion">
                    <?php if($classifyLists): if(is_array($classifyLists)): foreach($classifyLists as $key=>$v): ?><div class="panel panel-default maxWarp">
                                <div class="panel-heading">
                                    <h4 class="panel-title">
                                 <a href="#collapse<?php echo ($key+1); ?>" data-toggle="collapse" data-parent="#accordion"><?php echo ($v["name"]); ?><span class="glyphicon glyphicon-chevron-up sanjiao"></span></a>
                            </h4>
                                </div>
                                <div id="collapse<?php echo ($key+1); ?>" class="panel-collapse collapse liwarp">
                                    <div class="panel-body">
                                        <ul class="lists">
                                            <input type="hidden" value="<?php echo ($nowCid); ?>" id="nowCid">
                                            <?php if($v["child"]): if(is_array($v["child"])): foreach($v["child"] as $key=>$value): ?><li cid='<?php echo ($value["id"]); ?>'>
                                                        <div class="media">
                                                            <div class="media-left">
                                                                <a href="<?php echo U('lists',array('cid'=>$value['id']));?>">
                                                    <img src="<?php echo ($value["faceurl"]); ?>" class="media-object">
                                                </a>
                                                            </div>
                                                            <div class="media-body">
                                                                <a href="<?php echo U('lists',array('cid'=>$value['id']));?>"><?php echo ($value["name"]); ?></a>
                                                            </div>
                                                        </div>
                                                    </li><?php endforeach; endif; ?>
                                                <?php else: ?>
                                                <h2>暂木有数据</h2><?php endif; ?>
                                        </ul>
                                    </div>
                                </div>
                            </div><?php endforeach; endif; ?>
                        <?php else: ?>
                        <h1>暂时木有数据</h1><?php endif; ?>
                </div>
            </div>
            <!-- 主页面 就是右侧列表-->
            <div class="col-md-10 p0 main">
                <div class="row">
                    <p class="col-md-12">奇迹热歌</p>
                    <div class="col-md-12">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                <?php if($resultAll): if(is_array($resultAll)): foreach($resultAll as $key=>$v): ?><tr>
                                            ...
                                            <td><a href="#"><?php echo ($v["musicname"]); ?></a></td>
                                            <td><a href="#"><?php echo ($v["author"]); ?></a></td>
                                            <td class="hidden-xs"><span class="glyphicon glyphicon-fire"></span> <?php echo ($v["playcount"]); ?></td>
                                            <td class="OP">
                                                <span class="glyphicon glyphicon-plus"></span>
                                                <span class="glyphicon glyphicon-play" mid='<?php echo ($v["id"]); ?>'></span>
                                            </td>
...
$(".jAudio--player").jAudio(musicData);  
    ...
    <!-- 音乐详细页js代码 -->
    <script type="text/javascript" src="/Public/js/Index/M_lists.js"></script>
</body>
