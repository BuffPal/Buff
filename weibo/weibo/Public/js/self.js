 ...
 $("body").on('click','a.keep',function(){
  ...
  $('#face').uploadify({
    ...
    onUploadSuccess:function(file,data,response){     //上传成功后的回调函数 只需要用data  是php返回的
        eval('var data =' + data);
        if(data.status){
          $('input[name=max]').val(data.facePath);
          $('input[name=md]').val(data.pathmd);
          $('input[name=mini]').val(data.pathmini);
          $picHtml = '<li><img src="'+ROOT+data.pathmini+'"></li>';
          $('#hiddenPic').show();
          $('#hiddenPic').append($picHtml);
          $('#hiddenPic img').animate({'height':50},200);
        }else{
          alert(data.msg);
        }
      }
  ...
  //发布框 输入表情
  var oPhizBox = $('#icon-happy');
  var oPhiz    = $('#hiddenPhiz img');
  var oPhizX   = $('#icon-cross');
    oPhizBox.click(function(){//所有的点击I标记 显示 表情选择框
    var x = $(this).position().left;
    var y = $(this).position().top;
    $('#hiddenPhiz').css({
      'top':y+30,
  ...
  //AJAX 处理 提交发布微博 插入后添加节点
  $('#sendWB').click(function(){
    var oContent = $('textarea[name=operationContenti]');
    var sContent = oContent.val();
    if(trim(sContent) == ''){//如果输入内容为空 不进行下面操作
      flashingRed(oContent);//Tool.js 里面定义的闪烁红光效果
      oContent.focus();
      return false;
    }
    var sMax  = $('input[name=max]').val();
    var sMd   = $('input[name=md]').val();
    var sMini = $('input[name=mini]').val();
    var iNum  = $('p.userWeibo').html();
    $.post(addWBUrl,{
      'content':sContent,
      'max':sMax,
      'medium':sMd,
      ...
      if(data.status){
        ...
        tishi('发布成功');
        //显示
        $('li.liClassanimate').fadeIn();
        ///同上结束
        //在自己微博的数量上加1
        $('p.userWeibo').html(parseInt(iNum)+1);
      }else{
        alert(data.msg);
      }
    },'json');
...
$(document).ready(function(){//这里有BUG ajax会一直读取不知道为啥
  ...
  $(document).scroll(function(){
    ...
    var wHeight = document.documentElement.scrollTop || window.pageYOffset || document.body.scrollTop;//滚动上的距离
    if(wHeight>400){
      $('#return').fadeIn();
    }else{
      $('#return').fadeOut();
    }
    var limit = $('#blogUl>li').length;//获取当前页面的总共有多少条微博
    if(sTop>(DHeight*0.9)&&on&&limit >= 10){//这个是判断 滚动条到哪里的  如果limit 不等于10就不给他显示,应为配置设置的默认十条
      on = false;//这个很关键, 第一次就把开关关闭了 解决滚动条下拉发送多条AJAX
      $.ajax({
        url : ajaxDataUrl,
        data : {
          'limit':limit,
          gid : $('input[name=nowGroup]').val()
        },
        type : 'post',
        dataType: 'json',
        beforeSend : function(){//加载
          $('#scrollAjax').fadeIn(200);
        },
        success : function(data){
          if(data.status){
            $('#blogUl').append(data.html);
            on = true;
          }else{
