...
<head>
  ...
  <script type="text/javascript">
    ...
    var uploadFace     = "<?php echo U('Common/uploadPic');?>";//头像上传处理方法
    var sid            = '<?php echo session_id();?>';//这里是因为 uploadify不传session过去,而处理地址需要用户登录
    var addWBUrl       = "<?php echo U('sendWeiBo');?>";//AJAX发布微博地址
    var ajaxDataUrl    = "<?php echo U('ajaxData');?>";//AJAX滚动条 加载数据地址
    ...
    var deleteWbUrl    = "<?php echo U('deleteWb');?>";//删除微博AJAX处理
  </script>
  <script type="text/javascript" src="/weibo/Public/js/self.js"></script>
</head>
<body>
  <!-- 头部模板 -->
  ...
  <div id="main">
    <div id="sdebar">
      ...
      <ul class="bottom" id="ulBottom">
        ...
        <li><i class="icon-user-tie"></i><a href="#" gid="0">全部</a></li>
<?php if(is_array($group)): $i = 0; $__LIST__ = $group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li><i class="icon-user-tie"></i><a href="#" gid="<?php echo ($v["id"]); ?>"><?php echo ($v["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
      </ul>
    ...
    <div id="mainRBox">
      <div id="mainWB">
        <div class="release">
          <div class="title">
            <p>
            <span id="lb" style="top:100px;left:100px;position:fixed;color:#fff;"></span><!--一个固定的span标记 滚动时方便查看-->
              <font class="spac1">有什么新</font>
              <font class="spac2">鲜</font>
              <font class="spac3">事想告诉大家</font>
              <font class="spac4">?</font>
            </p>
            <span>您可以输入[&nbsp;<i id="font-num">140</i>&nbsp;]字</span>
          </div>
          <textarea name="operationContenti" sign='topWB'></textarea>
          <div class="sendBox">
            <span id="phiz"><i class="icon-happy" id="icon-happy"  sign='topWB'></i>
            
            </span>
              <span id="picture"><i class="icon-images"></i>
                <div id="pictureHiddenBox">
                  <span><h1>本地上传</h1><i class="icon-cross"></i></span>
            ...
            <div class="send">
              <input type="submit" value="发布" id="sendWB">
            </div>
          </div>
          <ul id="hiddenPic">
         ...
         <article class="excerpt">
          <header>
            <h4 class="userIFM" value="<?php echo ($vo['uid']); ?>">
              ...
              <a class="deleteWB" wid="<?php echo ($vo['id']); ?>">删除</a>
            </h4>
...
<?php if($vo['isturn']['mini']): ?><img src="/weibo<?php echo ($vo["isturn"]["max"]); ?>" data-action="zoom"><?php endif; ?>
            </div>
...
<?php else: ?>
  <span id="notWB">暂时没有内容~</span><?php endif; ?>     <!-- 是否存在数据结束 -->
      ...
      <div id="scrollAjax"><i class="icon-spinner2" ></i></div>
    </div>
<!-- 整体微博结束 -->
