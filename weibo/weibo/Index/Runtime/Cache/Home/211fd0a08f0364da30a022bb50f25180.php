...
<head>
  ...
  <script type="text/javascript">
    var deleteCommentUrl = "<?php echo U('deleteComment');?>";
  </script>
  <script type="text/javascript" src="/weibo/Public/js/commentList.js"></script>
</head>
<body>
  <!-- 头部模板 -->
  ...
  <div id="changebgBox">
   ...
   <link rel="stylesheet" type="text/css" href="/weibo/Public/Theme/<?php echo ($style); ?>/css/commentList.css">
  <div id="main">
    <div id="sdebar">
      <ul class="top">
...
<?php if(is_array($group)): $i = 0; $__LIST__ = $group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li><i class="icon-user-tie"></i><a href="<?php echo U('Self/index');?>" gid="<?php echo ($v["id"]); ?>"><?php echo ($v["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
    ...
    <div id="mainRBox">
      <div id="mainWB">
        <h1 class="callMeListTitle"><span>您有 <strong><?php echo ($callCount); ?></strong> 条评论</span></h1>
        <ul id="callMeList">
        <?php if($callAll): if(is_array($callAll)): foreach($callAll as $key=>$vo): ?><!-- 循环私信-->
            <li>
              <div class="face">
                <a href="<?php echo U('/'.$vo['uid']);?>">
                <img src="<?php if($vo['face']): ?>/weibo<?php echo ($vo['face']); else: ?>/weibo/Public/images/none.png<?php endif; ?>">
                </a>
              </div>
              <div class="content">
                <span class="name"><a href="<?php echo U('/'.$vo['uid']);?>"><?php echo ($vo['username']); ?></a></span>
        ...
        <?php else: ?>
          <h1 class="notData">您还没有评论哦~</h1><?php endif; ?>
