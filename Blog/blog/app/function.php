...
function getSize($sizeb)
{
    ...
    if ($sizemb > 1) {
        $size = round($sizemb , 2) . "M";
    }
    if ($sizegb > 1) {
        $size = round($sizegb , 2) . "G";
    }
    if ($sizetb > 1) {
        $size = round($sizetb , 2) . "tb";
    }
    if ($sizepb > 1) {
        $size = round($sizepb , 2) . "pb";
    }
    return $size;
}
 ...
 */
function replace_weibo($content , $a = true)
{//这个是模板里面调用的 所以就不能用&
...
function unlimitedForLevel($cate , $html = '——' ,  $pid = 0 , $level = 0 )
{
    $arr = array();
    foreach ($cate as $v) {
        if ($v['pid'] == $pid) {//第一次运行的时候是判断是不是祖先级元素，因为没有父级
            $v['level'] = $level + 1;
...
function getParents($cate , $id)
{
    ...
    foreach ($cate as $v) {
        if ($v['id'] == $id) {
            $arr[] = $v;
            $arr = array_merge(getParents($cate , $v['pid']) , $arr);//这是一个数组合并的函数，默认第一个在最前面,能达到更改foreach的指针熟顺序
        }
    }
    return $arr;
}
